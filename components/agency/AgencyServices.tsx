  ...
  return (
    <section id="services" className="relative py-12 sm:py-16 bg-[#02050e] overflow-hidden select-none">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 space-y-6 sm:space-y-8">
        ...
        {/* Balanced 2x2 Bento Grid (Matching Mockup) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-stretch">
          {AGENCY_SERVICES.map((service) => {
            return (
              <div
                key={service.id}
                className="relative rounded-2xl p-6 sm:p-7 bg-[#070b16] border border-white/[0.08] hover:border-[#00d2ff]/40 transition-all duration-300 flex flex-col justify-between space-y-5 group hover:shadow-[0_20px_50px_rgba(0,0,0,0.6),0_0_30px_rgba(0,210,255,0.1)]"
              >
                <div className="space-y-4">
                  {/* Circular Cyan Icon Badge */}
                  <div className="w-11 h-11 rounded-full bg-[#00d2ff]/15 border border-[#00d2ff]/30 flex items-center justify-center text-[#00d2ff] shadow-[0_0_15px_rgba(0,210,255,0.2)] group-hover:scale-105 transition-transform">
                    {getSubtleIcon(service.iconName)}
                  </div>

                  {/* Title & Description */}
                  <div className="space-y-1.5">
                    <h3 className="text-lg sm:text-xl font-bold text-white group-hover:text-[#00d2ff] transition-colors tracking-tight">
                      {service.title}
                    </h3>
