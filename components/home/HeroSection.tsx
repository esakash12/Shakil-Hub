...
export default function HeroSection({ cms }: HeroSectionProps = {}) {
  ...
  return (
    <section className="relative overflow-hidden bg-black py-8 sm:py-16 lg:py-20 min-h-[350px] sm:min-h-[440px] lg:min-h-[500px] flex items-center">
      ...
      {/* Hero Background Image (Preloaded & Server-Rendered for <1s LCP) */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        {/* Hero Content - Focused, sleek editorial typography with GPU CSS animations */}
        <div className="max-w-xl lg:max-w-2xl space-y-3.5 sm:space-y-4 text-left">
          {/* Pill Tag */}
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-[11px] sm:text-xs font-semibold tracking-wide shadow-[0_0_12px_rgba(6,182,212,0.12)]">
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-ping" />
            <span>{data.heroPill}</span>
          </div>

          {/* Headline */}
          <h1 className="animate-in fade-in slide-in-from-bottom-3 duration-500 text-2xl sm:text-3xl md:text-4xl lg:text-[42px] font-extrabold text-white tracking-tight leading-tight lg:leading-[1.18]">
          ...
          {/* Subtext */}
          <p className="animate-in fade-in slide-in-from-bottom-3 duration-500 text-xs sm:text-sm md:text-[15px] text-zinc-300 max-w-md sm:max-w-lg leading-relaxed font-normal">
            {data.heroSubtext}
          </p>

          {/* Modern CTA Buttons */}
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-wrap items-center gap-3 pt-2 sm:pt-3">
            <Link
